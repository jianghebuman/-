...
可用 Navicat Premium 17 执行该脚本，脚本会自动创建数据库 `campus_recruitment`，并初始化 37 张业务表、字典、测试账号和演示数据。

当前数据库连接默认指向 `campus_recruitment`。当前演示库口径为 37 张表、434 个字段，约包含 64 家企业、77 个企业 HR、72 名学生、80 个岗位、101 条投递、18 条面试通知、20 条 Offer、54 条人才库记录。如果本地数据库已经跑过一段时间，`operation_log`、`system_notice` 等业务表的记录数可能会高于初始化脚本，这是正常的运行数据漂移；如需恢复初始演示数据，重新导入该脚本即可。
